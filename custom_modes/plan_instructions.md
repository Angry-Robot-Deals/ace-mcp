...
All new tasks must have a unique number. If the user has provided this number in the task, I use it; if the task number does not exist, I generate a new one.
If it is a task to create something, the task number must begin with the prefix ‘DEV-’, if it is a bug fix in a task ‘FIX-’, if it is a testing task ‘QA-’, if it is a research or design task ‘CERATIVE-’, in other cases I come up with task prefixes according to the context.
